      ...
      <div className="flex-1 flex">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900">{t("my_registered_fields")}</h1>
              <p className="text-xs sm:text-sm text-gray-600">{t("green_valley_farm")}</p>
            </div>
              ...
              <Card key={field.id} className="border-emerald-200 shadow-md bg-white overflow-hidden">
                ...
                <CardContent className="p-6 space-y-4 text-xs sm:text-sm">
                  <div className="grid grid-cols-2 gap-3 text-gray-700 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <div>
                      <span className="text-gray-400 block text-[11px]">{t("area_size")}</span>
                      <span className="font-bold text-gray-900">{field.area} Acres</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[11px]">{t("soil_type")}</span>
                      <span className="font-bold text-gray-900">{field.soil_type}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[11px]">{t("growth_stage_label")}</span>
                      <span className="font-bold text-emerald-800">{field.growth_stage}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-[11px]">{t("risk_score")}</span>
                  ...
                  <div className="space-y-2">
                    <h5 className="text-xs font-bold uppercase tracking-wider text-gray-800">{t("visual_health_timeline")}</h5>
                    <div className="flex items-center justify-between text-xs text-gray-600 bg-emerald-50/60 p-3 rounded-xl border border-emerald-100">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{t("last_scan")}: <strong>Tomato Early Blight</strong> (Moderate)</span>
                      </div>
                      <span className="text-[11px] text-gray-400">2 {t("days_ago")}</span>
                    </div>
                  </div>
